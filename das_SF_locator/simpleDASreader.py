...
def load_DAS_files(
    ...
    for n, filepath in enumerate(filepaths):
        ...
        # Use h5pydict to read the data -> f
        with h5pydict.DictFile(filepath, "r") as f:
            ...
            if n == 0:
                ...
                # Declare a numpy array of zeroes with the right size for the data
    ...
    # Lea useful here:
    # Optional: unwrap the data if unwr and the header "spatialUnwrRange" are true
    # Note: Sensitivity is 9.36e+06 rad/m/strain
    if unwr and m["header"]["spatialUnwrRange"]:
        data = unwrap(data, m["header"]["spatialUnwrRange"] / sensitivity, axis=1)

    # Lea useful here:
    # Optional: Spike thresholding
    if spikeThr is not None:
        data[np.abs(data) > spikeThr / sensitivity] = 0

    # Lea useful here:
    # Integration in time to go from strain rate to strain
    if integrate:
